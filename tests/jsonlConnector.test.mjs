...
// Local slash commands (/model, /compact, …) and their synthetic assistant
// echoes never start a turn: no hooks fire, so no Stop would ever clear a
// 'working' they fabricate. They must not touch status or lastConnectorTs
// (the freshness clock toJSON arbitrates against a real Stop hook).

test('0399: <command-name> user record → no working, no clock bump, sys tail line', () => {
  const a = makeAgent();
  const changed = parseEvent({ type: 'user', message: { role: 'user',
    content: '<command-name>/model</command-name>\n<command-message>model</command-message>\n<command-args>opus</command-args>' } }, a);
  assert.equal(changed, true); // tail line pushed
  assert.equal(a.status, 'idle');
  assert.ok(!a.lastConnectorTs);
  assert.equal(a.messageCount || 0, 0);
  assert.equal(a.tail.length, 1);
  assert.equal(a.tail[0].kind, 'sys');
  assert.match(a.tail[0].text, /local command: \/model/);
});

test('0399: caveat + local-command-stdout records are pure skips', () => {
  const a = makeAgent();
  for (const content of [
    '<local-command-caveat>Caveat: The messages below were generated by the user while running local commands.</local-command-caveat>',
    '<local-command-stdout>Set model to Fable 5 and saved as your default</local-command-stdout>',
  ]) {
    const changed = parseEvent({ type: 'user', message: { role: 'user', content } }, a);
    assert.equal(changed, false);
  }
  assert.equal(a.status, 'idle');
  assert.ok(!a.lastConnectorTs);
...
test('0399: synthetic assistant echo → no working, no resolvedModel pollution', () => {
  const a = makeAgent();
  const changed = parseEvent({ type: 'assistant', message: {
    model: '<synthetic>', id: 'synth-1', stop_reason: 'stop_sequence',
    content: [{ type: 'text', text: 'No response requested.' }],
  } }, a);
  assert.equal(changed, false);
  assert.equal(a.status, 'idle');
  assert.equal(a.resolvedModel, null);
  assert.ok(!a.lastConnectorTs);
});

test('0399: /clear is exempt — reset still runs (and may bump the clock)', () => {
  const a = makeAgent({ context: 50000, status: 'working' });
  const changed = parseEvent({ type: 'user', message: { role: 'user',
    content: '<command-name>/clear</command-name>' } }, a);
  assert.equal(changed, true);
  assert.equal(a.status, 'idle');
  assert.equal(a.context, 0);
});

test('0399: a plain prompt merely MENTIONING the tags is still a real prompt', () => {
  const a = makeAgent();
  parseEvent({ type: 'user', message: { role: 'user',
    content: 'why does <command-name> appear in my transcript?' } }, a);
  assert.equal(a.status, 'working');
  assert.ok(a.lastConnectorTs > 0);
});
